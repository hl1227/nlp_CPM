...
if __name__ == '__main__':
    ...
    parser.add_argument('--max_len', default=800, type=int, required=False, help='生成的最长长度')
    parser.add_argument('--log_path', default='log/generate.log', type=str, required=False, help='日志存放位置')
    parser.add_argument('--no_cuda', action='store_true', help='不使用GPU进行预测')
    parser.add_argument('--model_path', type=str, default='model/youxi', help='模型存放位置')
    #parser.add_argument('--title', type=str, default='皇后成长计划详细攻略怎么做皇后', help='作文标题')
    #parser.add_argument('--context', type=str, default='皇后成长计划详细攻略怎么做皇后', help='作文上文')
    ...
    for keyword in f_keyword.readlines():
        while True:
            try:
                ...
                with open(f'result/0606/{title}.txt','w+',encoding='utf-8') as f_result:
                    f_result.write(result+'\n')
                    logger.info("result:{}\n".format(result))
                    break
            except KeyboardInterrupt as e:
                logger.error(f'错误:{keyword}:{e}')
                continue
